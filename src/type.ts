...
export interface CtrlDevPart<Data extends Record<string, any> = {}> {
    ...
    _requires$: BehaviorSubject<Map<string, boolean>>
    _disables$: BehaviorSubject<Map<string, boolean>>
}
...
export interface CtrlProtoPart<Data extends Record<string, any> = {}> {
    ...
    helper: CtrlHelper
    /** 设置必填 */
    required(path: string, value: boolean): void
    /** 必填 */
    required(path: string): boolean
    required$(path: string): Observable<boolean>
    /** 设置禁用 */
    disabled(path: string, value: boolean): void
    /** 禁用 */
    disabled(path: string): boolean
    disabled$(path: string): Observable<boolean>
}
